  ...
  async fetch(request, env) {
    ...
    try {
      ...
      const tasks = allPages.map(p => {
        ...
        const startDateObj = (pr['Start date'] || pr['Due date'] || {}).date || null;
        const endDateObj = (pr['End Date'] || {}).date || null;

        const start = startDateObj ? startDateObj.start : null;
        const end = endDateObj ? endDateObj.start : (startDateObj ? startDateObj.end || startDateObj.start : null);
